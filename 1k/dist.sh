...
function dist_lib {
    ...
    if [ $(($DIST_FLAGS & $DISTF_NO_INC)) = 0 ]; then
        ...
        if [ "$CONF_TEMPLATE" = "config.h.in" ] ; then
            mkdir -p ${DIST_DIR}/include/win/${INC_DIR}
    ...
    if [ ! $(($DIST_FLAGS & $DISTF_WIN)) = 0 ]; then
        ...
        mkdir -p ${DIST_DIR}/prebuilt/win/
        copy1k "install_win_x64/${LIB_NAME}/lib/*.lib" ${DIST_DIR}/prebuilt/win/
        copy1k "install_win_x64/${LIB_NAME}/bin/*.dll" ${DIST_DIR}/prebuilt/win/
    fi
